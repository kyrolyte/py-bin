...
def process_markdown_file(file_path):
    try:
        ...
        header_index = lines.index(header_match.group(0))

        if not header_match:
            print(f"Skipping {file_path}: No number found in first H1 header.")
            return
        
        description_value = lines[header_index + 2]
        if len(description_value) > 180:
            dv_cleaned = description_value[:180] + "..."
        else:
        ...
        for line in lines:
            if "weight:" in line and not title_added:
                updated_lines.append(f"title: \"{new_title} | Read the Complete Matthew Henry Bible Commentary Online\"\n")
                updated_lines.append(f"linkTitle: \"{extracted_number}\"\n")
                updated_lines.append(f"description: \"{dv_cleaned}\"\n")
                title_added = True
