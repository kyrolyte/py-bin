...
def process_footnotes(raw_text: str, footnote_counter: list):
    ...
    # Regex for \f ... \f* (typical USFM footnote span)
    # This is a simple pattern; depending on the source, you might need to adjust.
    pattern = r"\\f(.*?)\\f\*"

    def repl(match):
        inner = match.group(1)

        # Optional reference part: \fr
        fr_match = re.search(r"\\fr\s+([^\\]+)", inner)
        fr_text = fr_match.group(1).strip() if fr_match else ""

        # Main footnote text: \ft
        ft_match = re.search(r"\\ft\s+([^\\]+)", inner)
        if not ft_match:
            # If there's no \ft, drop the entire footnote block.
            return ""

        ft_text = ft_match.group(1).strip()

        # Build final footnote text: fr first (if present), then ft
        if fr_text:
        ...
        # Footnote definition (to be appended later)
    ...
    # Some USFM sources use \f ... \f (without \f*). If needed, add a similar regex.
    # pattern_simple = r"\\f(.*?)\\f"
    # new_text = re.sub(pattern_simple, repl, new_text)
...
def usfm_to_markdown(usfm_path: str, global_footnote_counter: list):
    # 1. Read file into memory (so we can grab \h before writing anything)
    with open(usfm_path, encoding="utf-8") as f:
        lines = f.readlines()

    # 2. Find \h for folder name
    header_name = None
    for line in lines:
    ...
    # 3. Iterate through lines and build markdown
    for line in lines:
        marker, text = extract_marker_and_text(line)

        # If it's a plain continuation line (no marker)
        if marker is None:
        ...
        # Section headings \s1, \s2, etc. -> ## heading
        ...
        # Reference line \r -> bold on its own line
        if marker == "\\r":
            if chapter_file:
                content = text.strip()
                if content:
        ...
        # Tags that should be appended to current verse line (e.g., \b, \pmo, \q, \q1, \q2)
    ...
    # Close the last chapter file, writing any remaining footnotes
    if chapter_file:
        if pending_footnotes:
            chapter_file.write("\n\n")
            for fn in pending_footnotes:
...
def main():
    ...
    # Global footnote counter that persists across all files
